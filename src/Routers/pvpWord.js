...
//    Located at src/Hendlers/better-words.js
 ...
 * It uses your existing `getWord(language, length, wordList)` from better-words.js.
 *
 * - lang     → "en" / "he"
...
async function pickWordForMatch({ matchId, lang, length }) {
  ...
  // const blocked = [...];
  // const value = await getWord(lang, length, blocked);
  const value = await req.getWord(lang, length, []);
...
// Export router + cache so sockets can clear words when match ends
module.exports = {
  pvpWordRouter: router,
  pvpWordCache
};
